...
- **Share sheet.** Saves go to your downloads, which on a phone is not the
  camera roll — the picture is on the device but nothing that shows
  photographs will show it. Turning on `Share sheet` hands the finished file
  to the system sheet instead, so Photos, Drive, Messages and the rest
  appear as destinations and the picture can land in your gallery. It's off
  unless you ask for it: a shutter press should take a photograph, not open
  a conversation about where to send it. Stills, GIFs, clips and every
